...
For PCA we are taking only 8 variables. We, choose these variables because we want to predict that what will the Mean salary looks like for any Major category. And these variables helps us to predict such.
  - Total
  - Employed
  - Full_time
  - Unemployed
...
```
```{r biplot 2}
biplot(results, scale = 0,  expand=1, xlim=c(-4.0, 2.0), ylim=c(-5.0, 2.0))
```
...
library(corrplot)
cor(batch)
corrplot.mixed(cor(batch[,-1]), order="hclust", tl.col="black")
```
```{r vif}
library(car)
temp <- PC
...
```
```{r predict}
predict(modelPC, se.fit = TRUE)
```

```{r vif}
library(car)
model1 <- lm(Median~Total+
               Employed+
               Full_time+
               Unemployed+
               College_jobs+
...
train <- batch[1:130,c("Median","Total","Employed","Full_time","Unemployed","College_jobs", "Non_college_jobs", "Low_wage_jobs")]
y_test <- batch[131:nrow(batch), c("Median")]
test <- batch[131:nrow(batch),c("Total","Employed","Full_time","Unemployed","College_jobs","Non_college_jobs", "Low_wage_jobs")]
```
...
model <-pcr(Median~Total+
               ...
               scale=TRUE,
               validation="CV")
pcr_pred <- predict(model, test, ncomp=3, se.fit = TRUE)
pcr_pred
```
...
nd = with(train, data.frame(Total=2000,
                            ...
                            Unemployed=37, 
                            College_jobs=1534,
                            Non_college_jobs=150,
                            Low_wage_jobs=53))
nd$pred = predict(model, newdata=nd, type="response", ncomp=3)
nd
```

Thank you for Reading. Let me know Professor if i am wrong somewhere, or have to add something in the report specifically. 
